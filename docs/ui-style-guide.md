...
This guide defines the default visual language for the XFile UI. The baseline direction is **White Frosted Command Center**: a compact file-management workspace with white translucent navigation, high-readability content surfaces, restrained iOS-style glass layers, and predictable motion.

The guide is intentionally token-driven. Values can be adjusted later without changing the product direction.

## Design Direction

- Style name: XFile Command Center
- Keywords: compact console, white frosted navigation, soft depth, clear hierarchy, high readability
...
## XFile Component Strategy

XFile uses project-owned React components and token-driven native controls. Third-party UI suites are not part of the visual foundation.

- Shared controls live in `web/src/components/ui.tsx`.
- Global appearance state lives in `web/src/appearance.tsx`.
- Theme, radius, and motion preferences must be expressed through CSS tokens, never page-specific hard-coded variants.
- Native inputs, tables, and links inherit the same tokens so pages remain visually consistent while specialized components are introduced gradually.
- High-frequency controls must expose accessible labels, focus states, disabled/loading states, and reduced-motion behavior.

### User-adjustable appearance

- Theme: `system`, `light`, `dark`.
- Radius: `compact`, `balanced`, `rounded`.
- Motion: `reduced`, `standard`, `expressive`.
- Preferences persist in local storage and apply to public pages, login, sharing, file management, and the admin console.
...
- Collapsed width: `76px`; persist the user's collapsed state locally.
- Item height: `40px`
- Item radius: `10px`
- Active item background: primary soft
- Active item text/icon: primary
- Inactive item text: muted
- The sidebar container itself uses square outer corners.
- Do not use a bright left-edge block or inset stripe for the active state.
...
- Header containers use square outer corners and attach to the viewport edge.
...
- Removing a row uses an opacity/translate exit animation while remaining rows stay mounted; do not reload the whole table after deletion.
- A multi-file or folder selection creates one aggregate share link and shows one QR code immediately.
- Direct-link creation is idempotent per storage source and file path.

Favorites And Home Navigation:

- Authenticated home pages expose My Shares, My Short Links, and My Favorites shortcuts.
- Users can choose which shortcuts are visible; persist this choice locally.
- Favorites are user-scoped, queryable, and removable.
- Right-clicking empty file-list space exposes upload, create-folder, and create-file actions.
...
`AppearanceProvider` maps saved preferences to document data attributes:

```html
<html data-theme="dark" data-radius="balanced" data-motion="standard">
```

`global.css` maps these attributes to color, radius, duration, easing, and press-feedback tokens. Components consume the tokens and must not branch on appearance preferences in page code.

## Migration Checklist

1. Replace new hard-coded colors, radii, and durations with shared tokens.
2. Keep reusable interaction logic in `components/ui.tsx`.
3. Verify focus, disabled, loading, empty, success, and error states.
4. Test all three radius settings for clipping and density changes.
5. Test reduced motion and the operating-system reduced-motion preference.
6. Verify public pages and admin pages in both light and dark modes.
7. Check 390px, 768px, 1024px, and 1440px viewports before delivery.
