...
export function normalizeExternalPreviewProvider(
  value?: string,
): ExternalPreviewProvider {
  const provider = value?.trim();
  if (provider === "kkfileview" || provider === "onlyoffice") return provider;
  return "disabled";
}
...
export function onlyOfficeDocumentType(
  ext?: string,
): OnlyOfficeDocumentType | undefined {
  return onlyOfficeDocumentTypes[(ext || "").toLowerCase()];
}

export function supportsExternalPreview(
...
export function buildExternalPreviewUrl(
  settings: Record<string, string>,
  ...
  if (template) return applyExternalPreviewTemplate(template, server, file);
  if (!server || provider !== "kkfileview") return "";
...
function applyExternalPreviewTemplate(
  template: string,
  server: string,
  file: ExternalPreviewFile,
) {
