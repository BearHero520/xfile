...
func TestSuperAdminOnlyManagementRoutes(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "create storage source", method: http.MethodPost, target: "/api/storage-sources", body: `{"name":"Alt","key":"alt","type":"local","rootPath":"` + filepath.ToSlash(t.TempDir()) + `","enabled":true}`},
		{name: "list users", method: http.MethodGet, target: "/api/users"},
		{name: "save settings", method: http.MethodPut, target: "/api/settings", body: `{"siteName":"Changed"}`},
		{name: "delete logs", method: http.MethodDelete, target: "/api/logs?all=true"},
	}
	...
	if res := performJSONRequestAs(s, "operator", http.MethodGet, "/api/storage-sources", ""); res.Code != http.StatusOK {
		t.Fatalf("normal admin should read assigned storage sources, got %d: %s", res.Code, res.Body.String())
	}
	if res := performJSONRequestAs(s, "operator", http.MethodGet, "/api/settings", ""); res.Code != http.StatusOK {
		t.Fatalf("normal admin should read operation settings, got %d: %s", res.Code, res.Body.String())
	}
	if res := performJSONRequestAs(s, "root", http.MethodGet, "/api/users", ""); res.Code != http.StatusOK {
