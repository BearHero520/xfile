...
func TestBatchMoveFilesRouteMovesSelectedFiles(t *testing.T) {
	...
	res := performJSONRequestAs(s, "root", http.MethodPatch, "/api/files/batch/move", `{"storageKey":"local","paths":["docs/a.txt","docs/b.txt"],"targetDir":"archive"}`)
...
func TestBatchCopyFilesRouteCopiesSelectedFiles(t *testing.T) {
	...
	res := performJSONRequestAs(s, "root", http.MethodPatch, "/api/files/batch/copy", `{"storageKey":"local","paths":["docs/a.txt","docs/b.txt"],"targetDir":"copies"}`)
	if res.Code != http.StatusOK {
