...
func TestUserSessionsCanBeListedAndRevoked(t *testing.T) {
	...
	listReq := httptest.NewRequest(http.MethodGet, "/api/users/"+strconv.FormatInt(user.ID, 10)+"/sessions", nil)
	...
	target := "/api/users/" + strconv.FormatInt(user.ID, 10) + "/sessions/" + strconv.FormatInt(otherSession.ID, 10)
...
func TestRevokingAllUserSessionsForcesLogout(t *testing.T) {
	...
	target := "/api/users/" + strconv.FormatInt(user.ID, 10) + "/sessions"
