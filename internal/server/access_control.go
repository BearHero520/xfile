...
func validateAccessSettings(settings map[string]string) error {
	...
	if value, ok := settings["webdavMountPath"]; ok {
		...
		if mountPath == "/" || mountPath == "/api" || strings.HasPrefix(mountPath, "/api/") || mountPath == "/share" || strings.HasPrefix(mountPath, "/share/") || mountPath == "/open" || strings.HasPrefix(mountPath, "/open/") {
