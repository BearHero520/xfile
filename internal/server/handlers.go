...
func (s *Server) createShare(w http.ResponseWriter, r *http.Request) {
	...
	share, err := s.store.CreateShareWithToken(req.Path, req.ExpiresAt, req.Password, customKey)
	if err != nil {
		writeError(w, http.StatusBadRequest, err)
		return
	}
...
func (s *Server) batchCreateShares(w http.ResponseWriter, r *http.Request) {
	...
	if !s.requireStoragePathAccess(w, r, "local", paths...) {
		return
	}
	shares := make([]domain.Share, 0, len(paths))
	for _, path := range paths {
		share, err := s.store.CreateShare(path, req.ExpiresAt, req.Password)
		if err != nil {
			writeError(w, http.StatusBadRequest, err)
			return
		}
	...
	_ = s.store.LogAccess("batch-share", strconv.Itoa(len(shares)), clientIP(r), r.UserAgent())
	writeJSON(w, http.StatusCreated, shares)
}
...
func (s *Server) downloadShare(w http.ResponseWriter, r *http.Request) {
	...
	share, path, err := s.store.SharedFilePath(token, password, r.URL.Query().Get("path"))
	if err != nil {
		s.recordSharePasswordFailure(r, token, password, err)
		writeError(w, http.StatusForbidden, err)
		return
...
func (s *Server) batchCreateDirectLinks(w http.ResponseWriter, r *http.Request) {
	...
	for _, path := range paths {
		link, err := s.store.CreateDirectLink(path)
		if err != nil {
			writeError(w, http.StatusBadRequest, err)
			return
		}
