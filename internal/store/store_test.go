...
func TestCustomShareKey(t *testing.T) {
	...
	if share.Token != "project-docs" || share.URL != "/share/project-docs" {
...
func TestShareAndDirectLinkUseSelectedStorageSource(t *testing.T) {
	s := newTestStore(t)
	root := filepath.Join(t.TempDir(), "team-drive")
	if err := os.MkdirAll(filepath.Join(root, "docs"), 0o755); err != nil {
		t.Fatalf("mkdir secondary source: %v", err)
	}
	if err := os.WriteFile(filepath.Join(root, "docs", "guide.txt"), []byte("secondary source"), 0o644); err != nil {
		t.Fatalf("write secondary file: %v", err)
	}
	if _, err := s.CreateStorageSource(domain.StorageSourceInput{
		Name:     "Team drive",
		Key:      "team-drive",
		Type:     "local",
		RootPath: root,
	...
	share, err := s.CreateSourceShare("team-drive", "docs", "", "")
	if err != nil {
		t.Fatalf("create source share: %v", err)
	}
	if share.StorageKey != "team-drive" {
		t.Fatalf("unexpected share storage key: %#v", share)
	}
	detail, err := s.ShareDetail(share.Token, "", "")
	if err != nil {
		t.Fatalf("source share detail: %v", err)
	}
	if detail.StorageKey != "team-drive" || detail.Type != "folder" || len(detail.Files) != 1 {
		t.Fatalf("unexpected source share detail: %#v", detail)
	}
	_, sharedDownload, err := s.SharedDownload(share.Token, "", "guide.txt")
	if err != nil {
		t.Fatalf("shared source download: %v", err)
	}
	defer sharedDownload.Reader.Close()
	body, err := io.ReadAll(sharedDownload.Reader)
	if err != nil || string(body) != "secondary source" {
		t.Fatalf("unexpected shared source body %q: %v", string(body), err)
	}

	link, err := s.CreateSourceDirectLink("team-drive", "docs/guide.txt")
	if err != nil {
		t.Fatalf("create source direct link: %v", err)
	}
	resolved, err := s.ResolveDirectLink(link.Token)
	if err != nil {
		t.Fatalf("resolve source direct link: %v", err)
	}
	if resolved.StorageKey != "team-drive" {
		t.Fatalf("unexpected direct link storage key: %#v", resolved)
	}
	directDownload, err := s.SourceDownload(resolved.StorageKey, resolved.Path, false)
	if err != nil {
		t.Fatalf("direct source download: %v", err)
	}
	defer directDownload.Reader.Close()
	directBody, err := io.ReadAll(directDownload.Reader)
	if err != nil || string(directBody) != "secondary source" {
		t.Fatalf("unexpected direct source body %q: %v", string(directBody), err)
	}
}
