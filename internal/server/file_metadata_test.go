...
func TestSaveFileMetadataRouteUpdatesDescription(t *testing.T) {
	...
	res := performJSONRequestAs(s, "root", http.MethodPut, "/api/files/metadata", `{"storageKey":"local","path":"docs/note.txt","description":"Ops handoff"}`)
	...
	res = performJSONRequestAs(s, "root", http.MethodGet, "/api/files?storageKey=local&path=docs", "")
	if res.Code != http.StatusOK {
...
func TestSaveFileMetadataRouteRequiresRenameOperation(t *testing.T) {
	...
	res := performJSONRequestAs(s, "root", http.MethodPut, "/api/files/metadata", `{"storageKey":"local","path":"docs/note.txt","description":"Blocked"}`)
